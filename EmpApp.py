...
def fetchdata():
    ...
    workinghours = request.form['workinghours']
    photo = request.files['photo']
    employee_data = searchEmp(employeeid)
    return render_template('employeeinfo.html', employeeid=employeeid, name=name, dob=dob, gender=gender, address=address, phone=phone, email=email, jobstatus=jobstatus, hiredate=hiredate, jobtitle=jobtitle, department=department, payRoll=payRoll, workinghours=workinghours, photo=photo)
