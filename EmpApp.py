...
def leave():
    ...
    insert_sql = "INSERT INTO leaveApp VALUES (%s, %s, %s, %s, %s, %s, %s)"
    cursor = db_conn.cursor()

    try:

        cursor.execute(insert_sql, (employeeid, name, dateFrom, toDate, phone, email, department))
        db_conn.commit()
    ...
    print("all modification done...")
    return render_template('OutputLeave.html', employeeid=employeeid, name=name, dateFrom=dateFrom, toDate=toDate, phone=phone, email=email, department=department )
